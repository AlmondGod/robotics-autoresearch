...
FROZEN_MANIFEST = "data/autorobobench/robocasa_microwave_peak_manifest.json"
FROZEN_SPLIT = "data/autorobobench/robocasa_microwave_peak_splits.json"


def main() -> None:
    _force_arg("--manifest", FROZEN_MANIFEST)
    _force_arg("--split", FROZEN_SPLIT)
    _default("--inference", "tasks.robocasa_microwave_peak.inference")
    _default("--max-steps", "300")
    _default("--commit-steps", "12")
...
def _rewrite_result(out: Path) -> dict | None:
    ...
    payload["track"] = "robocasa_microwave_peak"
