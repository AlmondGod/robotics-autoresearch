...
DEFAULT_MANIFEST = Path("data/autorobobench/robocasa_microwave_peak_manifest.json")
DEFAULT_SPLIT = Path("data/autorobobench/robocasa_microwave_peak_splits.json")
DEFAULT_VIDEO_POOL = Path("data/autorobobench/robocasa_microwave_peak_video_pool.json")
DEFAULT_VIEWS = ("robot0_agentview_left", "robot0_agentview_right")


def main() -> None:
    parser = argparse.ArgumentParser(description="Verify the RoboCasa microwave peak task assets.")
...
def verify_assets(manifest_path: Path, split_path: Path, video_pool_path: Path, *, verify_files: bool) -> dict:
    ...
    if video_pool.get("target_task_included") is not False:
        raise ValueError(f"{video_pool_path} must exclude the target microwave task")

    manifest_tasks = {task["alias"]: task for task in manifest["tasks"]}
    if set(manifest_tasks) != {"PickPlaceCounterToMicrowave"}:
        raise ValueError("microwave peak manifest must contain only PickPlaceCounterToMicrowave")
    ...
    for task in video_pool["tasks"]:
        ...
        if alias == "PickPlaceCounterToMicrowave":
